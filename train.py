...
from util.utils import make_weights_for_balanced_classes, get_val_data, separate_irse_bn_paras, \
                      ...
                      buffer_val, AverageMeter, accuracy, pil_loader_bgr2rgb
...
if __name__ == '__main__':
    ...
    train_transform = transforms.Compose([ # refer to https://pytorch.org/docs/stable/torchvision/transforms.html for more build-in online data augmentation
        ...
        transforms.ToTensor(),
        # transforms.Lambda(lambda imgs: imgs[(2,1,0),:,:]),
        transforms.Normalize(mean = RGB_MEAN,
                             std = RGB_STD),
    ])

    dataset_train = datasets.ImageFolder(os.path.join(DATA_ROOT, 'imgs'), train_transform, loader=pil_loader_bgr2rgb)
    ...
    start_epoch = int(os.path.basename(BACKBONE_RESUME_ROOT).split('_')[4])
    time_end = time.time()
    print('loading pretrainning model---time cost',time_end-time_start,'s')
    for epoch in range(start_epoch+1, NUM_EPOCH): # start training process
        ...
        # print("=" * 60)
        print("Epoch {}/{}, Evaluation:  CALFW Acc: {}, CPLFW Acc: {}".format(epoch + 1, NUM_EPOCH,  accuracy_calfw, accuracy_cplfw))
        print("=" * 60)
        ...
        print('Training time per epoch---time cost Epoch ', epoch, epoch_time_end-epoch_time,'s')
